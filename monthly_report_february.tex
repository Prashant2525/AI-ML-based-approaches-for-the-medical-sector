...
\begin{itemize}[leftmargin=*]
    \item Run baseline zero-shot VQA inference using pre-trained BLIP-2 model to establish performance benchmarks.
...
\begin{itemize}[leftmargin=*]
    \item Overall completion: $\sim$25\% (Literature review, dataset preparation, EDA, and preprocessing pipeline completed).
    \item Timeline on track with dataset exploration and initial pipeline development finished ahead of schedule.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item Run baseline zero-shot inference with BLIP-2 and evaluate performance across complexity levels.
    \item Fine-tune the selected multimodal model on Kvasir-VQA-x1 training data.
    ...
    \item Expected Deliverables: Baseline results, fine-tuned model checkpoint, initial uncertainty analysis.
\end{itemize}

\section{Student Individual Contribution}

\noindent\textbf{Sunit Soni (AP22110011494):}
Led the literature review on uncertainty estimation and hallucination mitigation techniques in vision-language models. Surveyed methods including Monte Carlo Dropout, evidential deep learning, and selective prediction frameworks. Contributed to defining the research question and identifying the evaluation metrics (Risk-Coverage Curve, Expected Calibration Error) for the project.
